...
class DCN(nn.Module):
    ...
    def __init__(self, args,data_name,clustered_data_name):
        super(DCN, self).__init__()
        self.data_name=data_name
        self.clustered_data_name=clustered_data_name
        self.args = args
    ...
    def fit(self, epoch, train_loader, verbose=True):
        for batch_idx, (data, _) in enumerate(train_loader):
            ...
            if verbose and batch_idx % self.args.log_interval == 0:
                ...
                print(msg.format(epoch, batch_idx,
                                 loss.detach().cpu().numpy(),
                                 rec_loss, dist_loss))
        df = pd.read_csv(self.data_name).drop(['TEST_TRAIN', 'Problem'], axis=1)
        class Dataset():
            def __init__(self, df):
                x = df.drop('bRate', axis=1).values
                y = df['bRate'].values
        ...
        df_loader = torch.utils.data.DataLoader(
            Dataset(df),
            batch_size=len(df), shuffle=False)
        for data, target in df_loader:
            batch_size = data.size()[0]
            data = data.view(batch_size, -1).to(self.device)
            with torch.no_grad():
                latent_X = self.autoencoder(data, latent=True)
            ...
            for k in range(self.args.n_clusters):
                ...
                if elem_count[k] == 0:
                    continue
                self.kmeans.update_cluster(latent_X[cluster_id == k], k)
            data2 = pd.read_csv(self.data_name)
            data2['cluster']=cluster_id
            data2.to_csv(self.clustered_data_name,index=False)
            break
